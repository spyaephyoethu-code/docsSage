...
async def _ingest(
    ...
    documents = await loader_cls().load(source.url_or_path)
    logger.info(f"[{source_id}] Loaded {len(documents)} documents")

    chunker_cls = _CHUNKERS.get(source.chunking_strategy or "fixed")
    chunks: list[Chunk] = chunker_cls().chunk(documents)
        ...
        [
            {
                ...
                "source_title": c.metadata.get("file_path"),
            }
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            await source_repo.update_status(
                source_id, "failed", error_message="Ingestion timed out after 10 minutes — please retry"
            )
        except Exception as exc:
            logger.exception(f"[{source_id}] Ingestion failed")
            await source_repo.update_status(
                source_id, "failed", error_message=str(exc)
            )
